<template>
  <div class="tagged-view">
    ...
    <div class="view-header">
      ...
      <div>
        <h2 class="view-title">避雷榜</h2>
        <p class="view-sub">收录用户反馈差、有争议或有召回记录的品牌，消费前请注意</p>
      </div>
    </div>
      ...
      <div class="brand-list">
        <div v-for="item in items" :key="item.brand_id" class="brand-card">
          ...
          <el-avatar
            ...
            @error="() => true"
          />
          <el-avatar v-else :size="44" shape="square" style="background:#f56c6c;font-size:18px">⚠️</el-avatar>
    ...
    <el-empty
      v-else
      description="暂无避雷标注数据，可在管理后台为品牌添加「避雷」标签"
      :image-size="100"
    />
  </div>
</template>
...
async function load() {
  loading.value = true
  try {
    const res = await api.getBrandBlacklist({
...
function tagType(tag) {
  if (['推荐','国货之光','高性价比'].includes(tag)) return 'success'
...
.filter-tag:hover { border-color: #f56c6c; color: #f56c6c; }
.filter-tag.active { background: #fef0f0; border-color: #f56c6c; color: #f56c6c; font-weight: 600; }
